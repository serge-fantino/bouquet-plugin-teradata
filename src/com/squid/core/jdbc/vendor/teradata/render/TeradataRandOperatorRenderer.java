 ...
 *
 * @author loivd Random function
 * @revision : Ticket : Teradata random doesn't support a high bound greater
 *           than 2^31 - 1
 */
public class TeradataRandOperatorRenderer extends BaseOperatorRenderer {
	public TeradataRandOperatorRenderer() {
	}

	@Override
	public String prettyPrint(SQLSkin skin, OperatorDefinition opDef, String[] args) throws RenderingException {
		if (args.length == 0) {
			return "(RANDOM(0,2147483647)/CAST(2147483647 as FLOAT))";
		} else if (args.length == 2) {
			String str = "RANDOM(low_bound, hight_bound)";
